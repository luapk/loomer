/**
 * End-to-end board test for Leo and the Dolphin.
 *
 * Phase 1: Generate reference stills for all 6 Bible entities (parallel).
 * Phase 2: Generate all 14 shot key frames with those refs as conditioning images.
 *
 * Output: /tmp/leo-board-NN.jpg for each shot.
 *
 * Usage:
 *   npx tsx scripts/test-leo-boards.ts                         # illustrated (watercolour)
 *   npx tsx scripts/test-leo-boards.ts --photoreal             # cinematic photoreal
 *   npx tsx scripts/test-leo-boards.ts <model>                 # custom model, illustrated
 *   npx tsx scripts/test-leo-boards.ts <model> --photoreal     # custom model, photoreal
 */
...
const PHOTOREAL = process.argv.includes('--photoreal');
const modelArg = process.argv.slice(2).find((a) => !a.startsWith('--'));
const model = modelArg ?? 'nano-banana-pro-preview';
...
const PHOTOREAL_STYLE =
  'Cinematic photoreal, British coastal naturalism. Shot by Roger Deakins. ' +
  'Film: Kodak Vision3 500T. Naturalistic warm whites, restrained saturation. ' +
  'Naturalistic lighting, motivated by visible practicals.';

const STYLE = PHOTOREAL ? PHOTOREAL_STYLE : WATERCOLOUR;
const prefix = PHOTOREAL ? 'photo' : 'illus';

// ─── Phase 1: Reference stills ─────────────────────────────────────────────

const REFS = {
  'CHAR-LEO': {
    path: `/tmp/leo-${prefix}-CHAR-LEO.jpg`,
  ...
  'CHAR-DOLPHIN': {
    path: `/tmp/leo-${prefix}-CHAR-DOLPHIN.jpg`,
  ...
  'LOC-PIER-COASTAL-AFTERNOON': {
    path: `/tmp/leo-${prefix}-LOC-PIER.jpg`,
  ...
  'LOC-OCEAN-SURFACE-PIERSIDE': {
    path: `/tmp/leo-${prefix}-LOC-OCEAN.jpg`,
  ...
  'PROP-KITE': {
    path: `/tmp/leo-${prefix}-PROP-KITE.jpg`,
  ...
  'PROP-SPOOL': {
    path: `/tmp/leo-${prefix}-PROP-SPOOL.jpg`,
...
const modeLabel = PHOTOREAL ? 'Photoreal' : 'Illustrated (Watercolour)';
console.log(`═══ Leo and the Dolphin — ${modeLabel} Storyboard ═══`);
...
const boardResults = await Promise.allSettled(
  SHOTS.map(async (shot) => {
    ...
    const outPath = `/tmp/leo-${prefix}-board-${String(shot.n).padStart(2, '0')}.jpg`;
...
console.log(`\nOutput files: /tmp/leo-${prefix}-board-01.jpg … /tmp/leo-${prefix}-board-14.jpg`);
